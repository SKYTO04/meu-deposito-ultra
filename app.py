...
# =================================================================
# 1. DESIGN PREMIUM - DARK PRESTIGE (TOTALMENTE ESTÁVEL)
# =================================================================
...
st.markdown("""
    ...
    .badge-admin { background-color: #238636; color: white; padding: 2px 8px; border-radius: 10px; font-size: 0.7em; }
    .badge-user { background-color: #388bfd; color: white; padding: 2px 8px; border-radius: 10px; font-size: 0.7em; }
    .welcome-banner {
...
# =================================================================
DB_FILES = {
    "prod": "p_data.csv", "est": "e_data.csv", "pil": "pil_data.csv",
    "usr": "u_data.csv", "cas": "c_data.csv", "tar": "t_data.csv", 
    "cat": "cat_data.csv", "patio": "pat_data.csv", "meta": "m_data.csv"
}
...
if not st.session_state['autenticado']:
    ...
    # --- 📦 ESTOQUE (RECUPERADO) ---
        ...
        with st.form("f_est"):
            ...
            sel = c1.selectbox("Item", df_p['Nome'].unique())
            op = c2.radio("Operação", ["ENTRADA", "SAÍDA"], horizontal=True)
            qtd = c3.number_input("Qtd Unidades", 1)
            if st.form_submit_button("Lançar"):
                if op == "SAÍDA": df_e.loc[df_e['Nome'] == sel, 'Estoque_Total_Un'] -= qtd
        ...
        for _, r in df_j.iterrows():
            ub, tipo = get_config(r['Nome'], df_p)
            f, a = r['Estoque_Total_Un'] // ub, r['Estoque_Total_Un'] % ub
            st.markdown(f'<div class="card"><b>{r["Nome"]}</b> ({r["Categoria"]})<br>{int(f)} {tipo}(s) e {int(a)} un. | <span style="color:#238636;">R$ {r["Estoque_Total_Un"]*r["Preco_Unitario"]:,.2f}</span></div>', unsafe_allow_html=True)

    # --- 🏗️ PILARES (RECUPERADO) ---
    elif menu == "🏗️ Pilares":
    ...
    # --- 🍶 CASCOS (RECUPERADO) ---
        ...
        with t2:
            for i, r in df_patio.iterrows():
                ...
                c_p1.write(f"**{r['Vasilhame']}:** {int(r['Total_Vazio'])} un")
                novo_p = c_p2.number_input("Ajuste", value=int(r['Total_Vazio']), key=f"p_{i}")
                if st.button("Salvar", key=f"btn_p_{i}"):
                    df_patio.at[i, 'Total_Vazio'] = novo_p; df_patio.to_csv(DB_FILES["patio"], index=False); st.rerun()
        with t3: st.table(df_cas[df_cas['Status']=="PAGO"].tail(10))

    # --- ✨ CADASTRO (RECUPERADO + REMOVER CATEGORIA) ---
    elif menu == "✨ Cadastro":
        st.title("✨ Central de Cadastro")
        ta1, ta2, ta3 = st.tabs(["➕ Novo Item", "📂 Categorias", "🗑️ Remover Item"])
        with ta1:
            with st.form("new_i"):
                n_i = st.text_input("Nome").upper().strip()
                c_i = st.selectbox("Categoria", sorted(list(set(["Romarinho", "Refrigerante"] + df_cat['Nome'].tolist()))))
                p_i = st.number_input("Preço", 0.0)
                if st.form_submit_button("Cadastrar"):
                    if n_i in df_p['Nome'].values: st.error("Já existe!")
                    else:
                        pd.concat([df_p, pd.DataFrame([[c_i, n_i, p_i]], columns=df_p.columns)]).to_csv(DB_FILES["prod"], index=False)
                        pd.concat([df_e, pd.DataFrame([[n_i, 0]], columns=df_e.columns)]).to_csv(DB_FILES["est"], index=False); st.rerun()
        with ta2:
            new_cat = st.text_input("Nova Categoria")
            if st.button("Adicionar"):
                pd.concat([df_cat, pd.DataFrame([[new_cat]], columns=df_cat.columns)]).to_csv(DB_FILES["cat"], index=False); st.rerun()
            st.divider()
            cat_del = st.selectbox("Remover Categoria", ["Selecione..."] + df_cat['Nome'].tolist())
            if st.button("Excluir Categoria") and cat_del != "Selecione...":
                df_cat[df_cat['Nome'] != cat_del].to_csv(DB_FILES["cat"], index=False); st.rerun()
        with ta3:
            item_del = st.selectbox("Item para APAGAR", ["Selecione..."] + df_p['Nome'].tolist())
    ...
    # --- 👥 EQUIPE (VISUAL AJUSTADO) ---
        ...
        for i, r in df_usr.iterrows():
            with cols_eq[i % 4]:
                pic = f"data:image/png;base64,{r['foto']}" if r['foto'] else "https://cdn-icons-png.flaticon.com/512/149/149071.png"
                badge, label = ("badge-admin", "ADMIN") if r['is_admin'] == "SIM" else ("badge-user", "EQUIPE")
                st.markdown(f'<div class="user-card"><img src="{pic}" class="avatar-round" width="90" height="90"><h4>{r["nome"]}</h4><p>@{r["user"]}</p><span class="{badge}">{label}</span></div>', unsafe_allow_html=True)
                if is_adm and r['user'] != 'admin':
                    if st.button(f"Remover {r['user']}", key=f"r_{r['user']}"):
    ...
    # --- 📋 TAREFAS ---
        ...
        if is_adm:
            with st.expander("Nova Tarefa"):
                nt = st.text_input("Tarefa")
                if st.button("Salvar T"):
        ...
        for i, r in df_tar.iterrows():
            st.markdown(f'<div class="card {"task-done" if r["Status"]=="OK" else ""}"><b>{r["Tarefa"]}</b></div>', unsafe_allow_html=True)
