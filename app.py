# --- 🏗️ PILARES (VERSÃO PROFESSIONAL) ---
        ...
        with st.expander("➕ MONTAR NOVA CAMADA (ESTRUTURA AMARRADA)"):
            ...
            if n_pilar:
                ...
                # Lógica de amarração: alterna entre 3x2 e 2x3 para estabilidade
        ...
        for pilar in df_pil['NomePilar'].unique():
            with st.container():
                ...
                # Ordenar camadas da maior para a menor (topo para base)
                ...
                for cam in camadas:
                    ...
                    cols = st.columns(5) # Grid de exibição
                    ...
                    for _, r in dados_cam.iterrows():
                        ...
                        with cols[int(r['Posicao'])-1]:
                            # Card Estilizado de cada Engradado no Pilar
                            card_html = f"""
                            <div style="background-color:#1c2128; padding:10px; border-radius:8px; border:1px solid #30363d; text-align:center; min-height:80px;">
                                <small style="color:#8b949e;">POS {r['Posicao']}</small><br>
                                <b style="font-size:0.9em;">{r['Bebida']}</b><br>
                                <span style="color:#238636; font-size:0.8em;">+{r['Avulsos']} un</span>
                            </div>
                            """
                    ...
                    st.markdown(f"<p style='text-align:right; color:#8b949e; font-size:0.8em;'>Subtotal Camada: {total_un_cam} un</p>", unsafe_allow_html=True)
                    st.divider()
