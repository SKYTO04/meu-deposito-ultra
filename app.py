...
# =================================================================
# 1. DESIGN PREMIUM - DARK PRESTIGE V215 (FIXED ANIMATION)
# =================================================================
...
st.markdown("""
    ...
    .welcome-text {
        font-size: 3em; font-weight: bold; color: #58a6ff;
        text-shadow: 0px 0px 15px rgba(88, 166, 255, 0.4);
    }
    .stButton>button {
                        ...
                        with st.container():
                            st.markdown(f"""
                                <div class="welcome-container">
                                    <div class="welcome-text">Bem-vindo, {match.iloc[0]['nome']}! 💎</div>
                                </div>
                            """, unsafe_allow_html=True)
                            with st.columns(5)[2]:
                                with st.spinner(""):
...
# =================================================================
    ...
    if menu == "🏠 Dashboard":
        ...
        c2.markdown(f'<div class="metric-card"><h4>Pátio</h4><h2>{int(df_patio["Total_Vazio"].sum())} un</h2></div>', unsafe_allow_html=True)
        c3.markdown(f'<div class="metric-card"><h4>Tarefas</h4><h2>{len(df_tar[df_tar["Status"]=="OK"])}/{len(df_tar)}</h2></div>', unsafe_allow_html=True)
    ...
    # --- ✨ CADASTRO ---
        ...
        with ta1:
            with st.form("f_i"):
                ...
                if st.form_submit_button("Cadastrar"):
                    if n_p in df_p['Nome'].values: st.error("Já cadastrado!")
                    else:
        ...
        with ta3:
            item_r = st.selectbox("Apagar", ["Selecione..."] + df_p['Nome'].tolist())
            if st.button("CONFIRMAR") and item_r != "Selecione...":
    ...
    # --- 📋 TAREFAS ---
        ...
        for i, r in df_tar.iterrows():
            is_ok = r['Status'] == "OK"
            st.markdown(f'<div class="task-card {"task-done" if is_ok else ""}"><b>{r["Tarefa"]}</b><br><small>{"✅ Por " + r["QuemFez"] if is_ok else "🟡 Pendente"}</small></div>', unsafe_allow_html=True)
            if not is_ok:
                if st.button("FEITO", key=f"tk_{i}"):
    ...
    # --- 🍶 CASCOS ---
        ...
        with t1:
            with st.form("f_c"):
                c1, c2, c3 = st.columns(3); cli = c1.text_input("Cliente").upper(); tip = c2.selectbox("Tipo", ["Romarinho", "600ml", "Coca 1L", "Coca 2L"]); q_c = c3.number_input("Qtd", 1)
                if st.form_submit_button("Lançar"):
        ...
        with t3:
            for v in ["Romarinho", "600ml", "Coca 1L", "Coca 2L"]:
                st.write(f"**{v}:** {df_patio[df_patio['Vasilhame']==v]['Total_Vazio'].values[0]} un")
    ...
    # --- 👥 EQUIPE ---
        ...
        if is_adm:
            with st.expander("Novo"):
                ...
                ui, ni, si, ai = u.text_input("User"), n.text_input("Nome"), s.text_input("Senha"), a.selectbox("Adm", ["NÃO", "SIM"])
                if st.button("Salvar"):
        ...
        for i, r in df_usr.iterrows():
            st.markdown(f'<div class="card-equipe"><b>{r["nome"]}</b> (@{r["user"]})</div>', unsafe_allow_html=True)
