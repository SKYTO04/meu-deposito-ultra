...
if not st.session_state['autenticado']:
    ...
    if menu == "🏠 Dashboard":
        ...
        st.divider(); st.subheader("📊 Logs Recentes"); st.table(pd.read_csv(DB_LOG).tail(5))

    # --- 📋 TAREFAS (COM RASTREIO) ---
    elif menu == "📋 Tarefas":
    ...
    # --- 📦 ESTOQUE PROFISSIONAL ---
        ...
        st.title("📦 Estoque e Inventário")
        st.dataframe(df_e, use_container_width=True)
        st.subheader("⚙️ Ajuste Manual (Entrada/Saída)")
        with st.container():
            sel_e = st.selectbox("Selecione o Produto", df_p['Nome'].unique())
            u_b, t_t = get_config_bebida(sel_e, df_p)
            c_m1, c_m2, c_m3 = st.columns(3)
            tipo_m = c_m1.radio("Operação", ["➕ ENTRADA (Compra/Carga)", "➖ SAÍDA (Ajuste/Quebra)"])
            qtd_f, qtd_u = c_m2.number_input(f"Qtd {t_t}s Full", 0), c_m3.number_input("Qtd Unidades Avulsas", 0)
            if st.button("CONFIRMAR AJUSTE NO ESTOQUE", use_container_width=True):
                ...
                df_e.to_csv(DB_EST, index=False); registrar_log(n_logado, f"Ajuste Estoque: {sel_e} ({total_ajuste} un)"); st.success("Estoque Atualizado!"); st.rerun()

    # --- 🍶 CASCOS (ESTORNO E RASTREIO) ---
    elif menu == "🍶 Cascos":
        st.title("🍶 Gestão de Vasilhames")
        t1, t2, t3, t4 = st.tabs(["🔴 Pendentes", "🚚 Saída Empresa", "📦 Saldo Pátio", "📜 Histórico"])
        with t1:
            with st.form("f_cas"):
        ...
        with t2:
            with st.form("f_emp"):
                e1, e2, e3 = st.columns([2, 2, 1]); emp, tiv, qtv = e1.text_input("Empresa").upper(), e2.selectbox("Vasilhame Levado", ["Romarinho", "Coca 1L", "600ml"]), e3.number_input("Qtd", 1)
                if st.form_submit_button("REGISTRAR SAÍDA"):
                    pd.concat([df_cas, pd.DataFrame([[f"OUT{datetime.now().microsecond}", datetime.now().strftime("%d/%m %H:%M"), f"SAÍDA: {emp}", "", tiv, -qtv, "PAGO", n_logado, datetime.now().strftime("%H:%M")]], columns=df_cas.columns)]).to_csv(DB_CAS, index=False); st.rerun()
        with t3:
            st.table(df_cas[df_cas['Status'] == "PAGO"].groupby('Vasilhame')['Quantidade'].sum().reset_index())
        with t4:
    ...
    # --- 👥 EQUIPE (COM FOTO E CADASTRO) ---
    elif menu == "👥 Equipe" and is_adm:
        st.title("👥 Gestão de Equipe")
        with st.expander("➕ ADICIONAR NOVO MEMBRO"):
            with st.form("f_equipe"):
                col_e1, col_e2 = st.columns(2)
                new_u, new_n = col_e1.text_input("Usuário (Login)"), col_e2.text_input("Nome Completo")
                new_s, new_a = col_e1.text_input("Senha", type="password"), col_e2.selectbox("Acesso", ["NÃO", "SIM"], format_func=lambda x: "Admin" if x=="SIM" else "Operador")
        ...
        for i, row in df_usr.iterrows():
            ...
            f_mem = f"data:image/png;base64,{row['foto']}" if not pd.isna(row['foto']) and row['foto'] != "" else "https://cdn-icons-png.flaticon.com/512/149/149071.png"
            c_img.markdown(f'<img src="{f_mem}" style="width:60px; height:60px; border-radius:50%; object-fit: cover;">', unsafe_allow_html=True)
            c_txt.markdown(f"**{row['nome']}** ({row['user']}) - {'⭐ Admin' if row['is_admin']=='SIM' else '🛠️ Operador'}")
            if row['user'] != 'admin' and c_btn.button("REMOVER", key=f"del_{i}"):
    ...
    # --- ⚙️ PERFIL ---
        ...
        if st.button("SALVAR FOTO") and upload:
            ...
            df_usr.loc[df_usr['user'] == u_logado, 'foto'] = b64; df_usr.to_csv(DB_USR, index=False); st.success("Foto salva!"); st.rerun()

    # --- 🏗️ PILARES (LÓGICA ORIGINAL) ---
    elif menu == "🏗️ Pilares":
        ...
        for p in df_pil['NomePilar'].unique():
            ...
            for cam in sorted(df_pil[df_pil['NomePilar']==p]['Camada'].unique(), reverse=True):
                st.caption(f"Camada {cam}"); cols = st.columns(5)
    ...
    # --- 📊 ADMIN FINANCEIRO ---
        ...
        st.divider(); c_a, c_b, c_c = st.columns(3)
        with c_a:
        ...
        with c_b:
            if st.button("📊 RELATÓRIO EXCEL"):
                output = io.BytesIO()
                with pd.ExcelWriter(output) as wr:
                    df_fin.to_excel(wr, sheet_name='Estoque'); df_cas.to_excel(wr, sheet_name='Cascos')
                st.download_button("⬇️ BAIXAR EXCEL", output.getvalue(), "relatorio.xlsx")
        with c_c:
            if st.button("🧹 LIMPAR CASCOS PAGOS"):
                df_cas[df_cas['Status'] == 'DEVE'].to_csv(DB_CAS, index=False); st.rerun()
    ...
    # --- ✨ CADASTRO ---
        ...
        with st.form("f_cad"):
            c1, c2, c3 = st.columns(3); cat = c1.selectbox("Cat", ["Romarinho", "Refrigerante", "Cerveja Lata", "Outros"]); nome, preco = c2.text_input("Nome").upper(), c3.number_input("Preço", 0.0)
            if st.form_submit_button("CADASTRAR"):
