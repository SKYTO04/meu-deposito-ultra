...
if not st.session_state['autenticado']:
    ...
    # Só acessamos as variáveis após garantir que o usuário está logado
    u_logado = st.session_state.get('u_l', 'desconhecido')
    ...
    # --- ABA: PILARES ---
        ...
        with st.expander("🆕 LANÇAR NOVA CAMADA"):
            ...
            cat_pilar = st.selectbox("Categoria do Pilar", df_p['Categoria'].unique())
            ...
            if n_pilar:
                ...
                if st.button("SALVAR CAMADA"):
                    regs = [[f"{n_pilar}_{c_atual}_{p}", n_pilar, c_atual, p, b, av_dict[p], cat_pilar] for p, b in beb_dict.items() if b != "Vazio"]
    ...
    # --- ABA: CASCOS ---
        ...
        st.title("🍶 Registro de Vasilhames")
        ...
        for i, r in df_cas[df_cas['Status'] == "DEVE"].iterrows():
            with st.container():
                ...
                if l2.button("PAGO", key=f"bx_{r['ID']}"):
