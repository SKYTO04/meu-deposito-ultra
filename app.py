...
# =================================================================
# 1. CONFIGURAÇÃO E ESTILO (VISUAL DARK PRESTIGE)
# =================================================================
...
# =================================================================
# 2. BANCO DE DADOS (BLINDAGEM TOTAL v31)
# =================================================================
DB = {
    "prod": "p_v31.csv", "est": "e_v31.csv", "pil": "pil_v31.csv",
    "usr": "u_v31.csv", "cas": "c_v31.csv", "tar": "t_v31.csv", 
    "cat": "cat_v31.csv", "patio": "pat_v31.csv"
}
...
def init_db():
    ...
    for f, c in conf.items():
        if not os.path.exists(f) or os.stat(f).st_size == 0:
            ...
            # Força a correção de colunas se o arquivo estiver bugado
...
if not st.session_state['autenticado']:
    ...
    # Carregamento Geral (Com proteção de erro)
    ...
    menu = st.sidebar.radio("Menu Principal", ["🏠 Início", "📦 Estoque", "🏗️ Pilares", "🍶 Cascos", "✨ Cadastro", "📋 Tarefas", "👥 Equipe"])
    ...
    if menu == "🏠 Início":
        ...
        if is_adm and not df_e.empty:
            ...
            c3.metric("Patrimônio Líquido", f"R$ {(df_v['Estoque_Total_Un']*df_v['Preco_Unitario']).sum():,.2f}")

    # --- 📦 ESTOQUE ---
    elif menu == "📦 Estoque":
        st.title("📦 Controle de Estoque")
        with st.expander("🔄 Movimento Manual"):
            with st.form("mov"):
    ...
    # --- 🏗️ PILARES (LÓGICA 3x2 / 2x3) ---
        ...
        with st.expander("🧱 Adicionar Nova Camada"):
            ...
            if n_pilar:
                cat_p = st.selectbox("Tipo de Carga", sorted(df_cat['Nome'].tolist()))
                ...
                cam_at = int(max_c) + 1
                atrav, frent = (3, 2) if cam_at % 2 != 0 else (2, 3) # Regra 3/2 Alternada
    ...
    # --- 🍶 CASCOS (COMPLETO: DEVEDORES, ESTORNO, TROCA) ---
    elif menu == "🍶 Cascos":
        ...
        with t1:
            with st.form("div"):
                cli, vas, q = st.text_input("Nome do Cliente").upper(), st.selectbox("Vasilhame", ["Romarinho", "600ml", "Coca 1L", "Coca 2L Retornável"]), st.number_input("Quantidade", 1)
                if st.form_submit_button("Lançar Dívida"):
                    new_c = pd.DataFrame([[f"C{datetime.now().microsecond}", datetime.now().strftime("%d/%m %H:%M"), cli, vas, q, "DEVE", ""]], columns=df_cas.columns)
                    pd.concat([df_cas, new_c]).to_csv(DB["cas"], index=False); st.rerun()
            st.divider()
        ...
        with t2:
            st.subheader("Histórico de Pagamentos e Estornos")
            if 'Status' in df_cas.columns:
                for i, r in df_cas[df_cas['Status']=="PAGO"].iterrows():
                    ch1, ch2 = st.columns([3, 1])
                    ch1.write(f"✅ {r['Cliente']} pagou {int(r['Quantidade'])} {r['Vasilhame']} (Recebido por: {r['QuemBaixou']})")
            ...
            with st.form("tr_emp"):
                ...
                if st.form_submit_button("Confirmar Saída"):
                    if df_patio.loc[df_patio['Vasilhame'] == v_tipo, 'Total_Vazio'].values[0] >= v_qtd:
                        ...
                        new_t = pd.DataFrame([[f"T{datetime.now().microsecond}", datetime.now().strftime("%d/%m"), emp, v_tipo, v_qtd, "TROCA", n_logado]], columns=df_cas.columns)
                        pd.concat([df_cas, new_t]).to_csv(DB["cas"], index=False)
                        st.success("Saída registrada!"); st.rerun()
                    else: st.error("Erro: Saldo insuficiente no pátio!")
    ...
    # --- ✨ CADASTRO ---
        ...
        with tab1:
            with st.form("cad_p"):
                n, c, pr = st.text_input("Nome do Produto").upper(), st.selectbox("Categoria", df_cat['Nome'].tolist()), st.number_input("Preço Unitário", 0.0)
            ...
            st.divider()
            exc = st.selectbox("Excluir Produto", df_p['Nome'].tolist())
            if st.button("REMOVER DEFINITIVAMENTE", type="primary"):
                df_p[df_p['Nome']!=exc].to_csv(DB["prod"], index=False)
                df_e[df_e['Nome']!=exc].to_csv(DB["est"], index=False); st.rerun()
        with tab2:
    ...
    # --- 📋 TAREFAS ---
        ...
        if is_adm:
            with st.form("tarefa"):
                desc = st.text_input("Tarefa/Obrigação")
                if st.form_submit_button("Adicionar"):
                    pd.concat([df_tar, pd.DataFrame([[f"T{datetime.now().microsecond}", desc, "PENDENTE", "DIÁRIA", ""]], columns=df_tar.columns)]).to_csv(DB["tar"], index=False); st.rerun()
        for i, r in df_tar[df_tar['Status'] == "PENDENTE"].iterrows():
            st.info(f"🔹 {r['Tarefa']}")
            if st.button("CONCLUÍDO", key=f"t_{i}"):
                df_tar.at[i, 'Status'] = "OK"; df_tar.to_csv(DB["tar"], index=False); st.rerun()

    # --- 👥 EQUIPE & PERFIL ---
    elif menu == "👥 Equipe":
        st.title("👥 Perfil e Colaboradores")
        st.markdown(f'<div class="profile-card"><img src="{src}" width="150" class="avatar-round"><h3>{n_logado}</h3><p>Acesso: {"👑 Admin" if is_adm else "🚀 Equipe"}</p></div>', unsafe_allow_html=True)
        with st.expander("📸 Atualizar Foto / Dados"):
            f = st.file_uploader("Subir foto quadrada")
            nova_s = st.text_input("Mudar Senha", type="password")
            if st.button("Salvar Alterações"):
                ...
                df_usr.to_csv(DB["usr"], index=False); st.success("Atualizado!"); st.rerun()
