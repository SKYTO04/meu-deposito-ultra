...
# =================================================================
# 1. ESTILO E CONFIGURAÇÃO (VISUAL PRESTIGE v45)
# =================================================================
...
# =================================================================
# 2. BANCO DE DADOS (v45 - PROTEÇÃO CONTRA TABELAS VAZIAS)
# =================================================================
DB = {
    "prod": "p_v45.csv", "est": "e_v45.csv", "pil": "pil_v45.csv",
    "usr": "u_v45.csv", "cas": "c_v45.csv", "tar": "t_v45.csv", 
    "cat": "cat_v45.csv", "patio": "pat_v45.csv", "log": "log_v45.csv"
}
...
if not st.session_state['autenticado']:
    ...
    if menu == "🏠 Início":
        ...
        c1.metric("Vazios no Pátio", f"{int(df_patio['Total_Vazio'].sum()) if not df_patio.empty else 0} un")
        ...
        c3.metric("Patrimônio Estoque", f"R$ {cap:,.2f}")
    ...
    # --- 🍶 CASCOS (FIX DATA E HORA / ESTORNO) ---
    elif menu == "🍶 Cascos":
        st.title("🍶 Vasilhames")
        ...
        with t3:
            ...
            df_trocas = df_cas[df_cas['Status']=="TROCA"]
            for _, r in df_trocas.iterrows(): st.text(f"🚚 {r['Data']} - {r['Cliente']} levou {int(r['Quantidade'])} {r['Vasilhame']} (Resp: {r['QuemBaixou']})")
    ...
    # --- ✨ CADASTRO ---
        ...
        with tab1:
            ...
            st.divider(); p_exc = st.selectbox("Apagar", df_p['Nome'].tolist())
        ...
        with tab2:
            ...
            if st.button("EXCLUIR CATEGORIA", type="primary"):
                if df_p[df_p['Categoria'] == c_exc].empty: df_cat[df_cat['Nome'] != c_exc].to_csv(DB["cat"], index=False); st.rerun()
                else: st.error("Categoria com produtos!")
    ...
    # --- 👥 EQUIPE ---
        ...
        if is_adm:
            ...
            with st.form("nu"):
                ...
                if st.form_submit_button("Cadastrar"): pd.concat([df_usr, pd.DataFrame([[lu, ln, ls, la, ""]], columns=df_usr.columns)]).to_csv(DB["usr"], index=False); st.rerun()
